...
function OfficialCamera() {
  ...
  const projectGraphicToggle = useSelector(
    (state) => state.projectGraphicToggle.value,
  );
  const timelineIntroToggle = useSelector(
    (state) => state.timelineIntroToggle.value,
  );
  const playmodeToggle = useSelector((state) => state.playmodeToggle.value);
  const { size } = useThree();

  const position = new THREE.Vector3(32.0, 0, 12.25);
  ...
  const centerCamera = () => {
    ...
    const distance = OfficialCameraRef.current.position.distanceTo(
      new THREE.Vector3(0, 0, 0),
    );
    const currentposition = OfficialCameraRef.current.position;
    const angle = OfficialCameraRef.current.position
      .clone()
      .normalize()
      .dot(rightDirection);
