...
const CreateSphereMaterials = async ({ renderer, month, uniforms }) => {
  try {
    ...
    const bumpTextures = await loadTexturesOther({
      renderer,
      list: bumpTexturePathGeneration(),
    });
    console.log("Bump Textures loaded:", bumpTextures.length);

    const roughnessTextures = await loadTexturesOther({
      renderer,
      list: RoughnessTexturePathGeneration(),
    });
...
function TestSplitSphereWeather() {
  ...
  const uniforms = useMemo(() => {
    ...
    lightColorValueConverter.setHSL(
      lightColorValue.h / 360,
      lightColorValue.s / 100,
      lightColorValue.l / 100,
    );
  ...
  useEffect(() => {
    if (memoizedSphereMesh.current) {
      if (Array.isArray(memoizedSphereMesh.current.material)) {
        memoizedSphereMesh.current.material.forEach((material) => {
          material.envMapIntensity = uniforms.envMapIntensity.value / 2000.0;
        });
      }
    }
  ...
  useEffect(() => {
    const disposeMaterials = () => {
      if (memoizedSphereMesh.current) {
        if (Array.isArray(memoizedSphereMesh.current.material)) {
          ...
          if (memoizedSphereMesh.current.material.map)
            memoizedSphereMesh.current.material.map.dispose();
          if (memoizedSphereMesh.current.material.bumpMap)
            memoizedSphereMesh.current.material.bumpMap.dispose();
          if (memoizedSphereMesh.current.material.roughnessMap)
            memoizedSphereMesh.current.material.roughnessMap.dispose();
    ...
    const updateMaterials = async () => {
      if (memoizedSphereMesh.current) {
        ...
        // LoadAllTextures();
        const sphereMaterials = await CreateSphereMaterials({
          renderer: gl,
          month: 1,
          uniforms: uniforms,
        });
  ...
  const updateTexture = async () => {
    ...
    loadTextures({ renderer: gl, month: bufferMonth })
      .then(async (nextTextures) => {
        try {
