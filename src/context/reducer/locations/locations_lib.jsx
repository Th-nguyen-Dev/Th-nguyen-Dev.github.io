...
const generate3DCoordinates = ({ locations, radius }) => {
  ...
  Object.keys(locations).forEach((key) => {
    coordinates[key] = get3DCoordinate({
      earthCoordinate: locations[key],
      radius,
    });
  });
  return coordinates;
...
const generateMovementQuaternions = ({ locations, endVec, radius }) => {
  const quaternions = {};
  Object.keys(locations).forEach((key) => {
    const currentStartVec = get3DCoordinate({
      earthCoordinate: locations[key],
      radius,
    });
    quaternions[key] = getMovementQuaternion({
      startVec: currentStartVec,
      endVec,
    });
  });
  return quaternions;
